...
{
   public class PageManagerLibrarianViewModel : PageManagerBaseViewModel<Librarian>
   {
      ...
      public PageManagerLibrarianViewModel()
      {
         ...
         SearchCommand = new RelayCommand<TextBox>((p) => p != null, (p) =>
         {
            ListDTO = LibrarianDAL.Instance.FindSimilar(p.Text, (EStatusFillter)StatusFillter);
         });
         ...
         ExportToExcelCommand = new RelayCommand<object>((p) => true, (p) =>
         {
            ...
            try
            {
               ...
               {
                  string[] columnHeaders = { "Mã số", "Họ và tên đệm", "Tên", "Giới tính", "Ngày sinh", "CCCD", "Địa chỉ", "Điện thoại", "Ngày bắt đầu", "Lương", "Trạng thái" };
                  double[] columnWidths = new double[] { 6, 20, 8, 7, 10, 15, 42, 12, 10, 10, 10 };
                  ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets[0];
                  StyleExcelWorkSheet(worksheet, "Thống kê danh sách thủ thư", columnHeaders, columnWidths);
         ...
         {
            if (EditLibrarianWindow.Show())
            {
               ShowSnackbarMessage("Thêm thủ thư thành công !");
               ReloadList();
            }
         ...
         {
            if (EditLibrarianWindow.Show(DTOSelected))
            {
               ShowSnackbarMessage("Chỉnh sửa thông tin thủ thư thành công !");
               ReloadList();
            }
            else { ShowSnackbarMessage("Không có thay đổi"); }
         });
      ...
      private void ReloadList() => ListDTO = LibrarianDAL.Instance.Gets(statusFillter);

      private void ChangeStatus()
      {
         if (LibrarianDAL.Instance.ChangeStatus(DTOSelected.UserId))
         {
